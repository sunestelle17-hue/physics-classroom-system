...
// --- Four-page simplified classroom flow ---
const simplifiedNormalizeState = normalizeState;
normalizeState = function(candidate = {}) {
  const base = simplifiedNormalizeState(candidate);
  return { ...base,
    page: ['home','prep','classroom','teacher'].includes(candidate.page) ? candidate.page : (candidate.currentView === 'teacher' ? 'teacher' : 'home'),
    prepDraft: candidate.prepDraft && typeof candidate.prepDraft === 'object' ? candidate.prepDraft : null,
    activePrepTab: candidate.activePrepTab || 'lesson',
...
function activeStudent() { return activeClass()?.students?.[state.studentIndex] || null; }
function makePrepDraft(mode = 'new') { return { mode, sessionName: mode === 'continue' ? '继续上节课堂' : '新课堂', plannedContent: '', lessonObjectives: '', selectedQuestions: [], sourceSessionId: '', createdAt: new Date().toISOString() }; }
function draftItems() { state.prepDraft ||= makePrepDraft(); return state.prepDraft.selectedQuestions ||= []; }
function questionKey(item) { return `${item.lessonId}:${item.moduleId}:${item.questionId || item.questionName}:${item.sourceSessionId || ''}`; }
function addDraftItems(items) { const existing = new Set(draftItems().map(questionKey)); let count = 0; items.forEach((raw) => { const item = { ...raw, sessionItemId: raw.sessionItemId || uid('si') }; if (!existing.has(questionKey(item))) { draftItems().push(item); existing.add(questionKey(item)); count++; } }); renumberDraft(); state.toast = count ? `已添加 ${count} 道题。` : '没有新增题目。'; }
function renumberDraft() { draftItems().forEach((item, i) => item.sessionOrder = i + 1); }
function lessonItems(lesson, sourceType = '按讲次添加') { return (lesson?.modules || []).flatMap(m => (m.questions || []).map((q, i) => sessionItemFromQuestion(lesson, m, q, i, sourceType))); }
function moduleDraftItems(lesson, module, sourceType = '按模块添加') { return (module?.questions || []).map((q, i) => sessionItemFromQuestion(lesson, module, q, i, sourceType)); }
function recentNonEmptySession(classId, excludeId = state.sessionId) { return (appData.sessions || []).filter(s => s.classId === classId && s.sessionId !== excludeId && (s.selectedQuestions || []).length).sort((a,b) => String(b.sessionDate || b.createdAt || '').localeCompare(String(a.sessionDate || a.createdAt || '')))[0] || null; }
function shouldContinueItem(session, item) { const students = (classById(session.classId)?.students || activeClass()?.students || []).map(s => s.id); return students.some(sid => { const q = session.studentRecords?.[sid]?.questions?.[item.sessionItemId]; return ['本节未讲到','下次继续','已布置但未完成','尚未检查'].includes(q?.incompleteStatus) || q?.continueNext === true; }); }
function continueCandidates(classId) { const last = recentNonEmptySession(classId); if (!last) return { last:null, items:[] }; const items = (last.selectedQuestions || []).map(item => ({ item, checked: shouldContinueItem(last, item) })); return { last, items }; }
function continuationClone(item, sourceSessionId, continuationType = '继续昨天课堂') { return { ...item, sessionItemId: uid('si'), sourceType: '承接上节课', sourceSessionId, continuationType, sessionOrder: 0 }; }
function selectedStats(items = draftItems()) { const lessons = new Set(items.map(i => i.lessonName).filter(Boolean)); const modules = new Set(items.map(i => `${i.lessonName}|${i.moduleName}`).filter(Boolean)); return { lessons: lessons.size, modules: modules.size, questions: items.length }; }
function showToast(message) { state.toast = message; persist(); render(); }
function navHtml() { const hasDraft = !!state.prepDraft; const hasSession = !!currentSession(); const items = [{p:'home',t:'首页',ok:true},{p:'prep',t:'准备',ok:hasDraft},{p:'classroom',t:'课堂登记',ok:hasSession},{p:'teacher',t:'教师工作台',ok:hasSession}]; return `<nav class="bottom-actions app-nav">${items.filter(i=>i.ok).map(i=>`<button data-page="${i.p}" class="${state.page===i.p?'active':''}">${i.t}</button>`).join('')}</nav>`; }
function toastHtml() { return state.toast ? `<div class="toast" role="status">${escapeHtml(state.toast)}</div>` : ''; }
function homeHtml(grade, klass, student, session) { return `<section class="card selectors home-only">${selectHtml('班级','class',state.classId,(grade?.classes||[]).map(c=>({value:c.id,label:c.name})))}${selectHtml('学生','student',state.studentIndex,(klass?.students||[]).map((s,i)=>({value:i,label:s.name})))}</section><section class="card"><h2>今天的课堂</h2><p>${session ? `${escapeHtml(session.sessionDate)} · ${escapeHtml(session.sessionName)} · ${escapeHtml(session.status)}` : '今天还没有选择课堂。'}</p><div class="home-actions"><button data-action="new-session">新建课堂</button><button data-action="continue-last">继续昨天课堂</button><button data-action="show-history">历史课堂</button><button data-action="toggle-manager">班级与学生管理</button></div></section>${state.managerOpen ? rosterManagerHtml(grade, klass) : ''}${state.historyOpen ? historyHtml() : ''}`; }
function prepSummaryHtml() { const stats = selectedStats(); return `<section class="card selected-summary prep-summary"><strong>已选</strong><span>讲次 ${stats.lessons}</span><span>模块 ${stats.modules}</span><span>题目 ${stats.questions}</span>${!stats.questions?'<span class="warn-inline">请选择题目后才能开始课堂</span>':''}</section>`; }
function prepHtml() { state.prepDraft ||= makePrepDraft(); const draft = state.prepDraft; const lessons = selectableLessons(); const cont = continueCandidates(state.classId); return `<section class="card"><h2>课堂准备页</h2><label class="note-label">课堂名称<input id="prep-session-name" value="${escapeHtml(draft.sessionName || '')}" placeholder="例如：速度图像专项"></label><label class="note-label">本节计划<input id="prep-plan" value="${escapeHtml(draft.plannedContent || '')}" placeholder="可选"></label><label class="note-label">本节目标<input id="prep-objectives" value="${escapeHtml(draft.lessonObjectives || '')}" placeholder="可选"></label></section>${prepSummaryHtml()}<section class="card"><div class="prep-tabs"><button data-prep-tab="lesson" class="${state.activePrepTab==='lesson'?'active':''}">按讲次添加</button><button data-prep-tab="module" class="${state.activePrepTab==='module'?'active':''}">按模块添加</button><button data-prep-tab="custom" class="${state.activePrepTab==='custom'?'active':''}">自定义选题</button><button data-prep-tab="continue" class="${state.activePrepTab==='continue'?'active':''}">从上一节继续</button></div>${state.activePrepTab==='lesson'?`<div class="click-list">${lessons.map(l=>`<button data-add-lesson="${escapeHtml(l.id)}"><strong>${escapeHtml(l.name)}</strong><span>${escapeHtml(classificationLabel(l))} · ${(l.modules||[]).reduce((n,m)=>n+(m.questions?.length||0),0)}题</span></button>`).join('') || '<p class="empty-summary">暂无可用讲次。</p>'}</div>`:''}${state.activePrepTab==='module'?`<div class="click-list">${lessons.flatMap(l=>(l.modules||[]).map(m=>`<button data-add-module="${escapeHtml(l.id)}:${escapeHtml(m.id)}"><strong>${escapeHtml(m.name)}</strong><span>${escapeHtml(l.name)} · ${m.questions?.length||0}题</span></button>`)).join('') || '<p class="empty-summary">暂无可用模块。</p>'}</div>`:''}${state.activePrepTab==='custom'?`<div class="question-picker">${lessons.map(l=>`<details><summary>${escapeHtml(l.name)}</summary>${(l.modules||[]).map(m=>`<fieldset><legend>${escapeHtml(m.name)}</legend>${(m.questions||[]).map((q,i)=>`<label class="check-chip"><input type="checkbox" data-custom-question="${escapeHtml(l.id)}:${escapeHtml(m.id)}:${i}">${i+1}. ${escapeHtml(questionTitle(q))}</label>`).join('')}</fieldset>`).join('')}</details>`).join('')}<button data-action="add-custom-selected">添加勾选题目</button></div>`:''}${state.activePrepTab==='continue'?continuePrepHtml(cont):''}</section><section class="card"><h2>已选内容</h2><div class="selected-list">${draftItems().map((item,i)=>`<div class="selected-row"><span><strong>${i+1}. ${escapeHtml(item.questionName)}</strong><small>${escapeHtml(item.lessonName)}｜${escapeHtml(item.moduleName)}</small></span><div><button data-move-draft="${i}:-1" ${i===0?'disabled':''}>上移</button><button data-move-draft="${i}:1" ${i===draftItems().length-1?'disabled':''}>下移</button><button data-remove-draft="${i}">删除</button></div></div>`).join('') || '<p class="empty-summary">尚未选择题目。</p>'}</div><button class="primary" data-action="confirm-start-session">确认开始课堂</button></section>`; }
function continuePrepHtml(cont) { if (!cont.last) return '<p class="empty-summary">当前班级没有可继续的非空历史课堂。</p>'; const unfinished = cont.items.filter(x=>x.checked).length; return `<div class="continue-box"><p><strong>${escapeHtml(cont.last.sessionDate || '')}</strong> · ${escapeHtml(cont.last.sessionName || '')}</p><p>讲次：${escapeHtml([...new Set((cont.last.selectedQuestions||[]).map(i=>i.lessonName))].join('、') || '未记录')}</p><p>模块：${escapeHtml([...new Set((cont.last.selectedQuestions||[]).map(i=>i.moduleName))].join('、') || '未记录')}</p><p>默认未完成题目数：${unfinished}</p><div class="question-picker">${cont.items.map(({item,checked})=>`<label class="check-chip"><input type="checkbox" data-continue-question="${escapeHtml(item.sessionItemId)}" ${checked?'checked':''}>${escapeHtml(item.moduleName)} · ${item.questionIndex}. ${escapeHtml(item.questionName)}</label>`).join('')}</div><button data-action="add-continue-selected">加入准备清单</button></div>`; }
function workspacePanel(id,title,html) { return `<section class="card teacher-panel"><button class="collapse-title" data-teacher-panel="${id}"><span>${title}</span><b>${(state.activeTeacherPanel||'feedback')===id?'收起':'展开'}</b></button>${(state.activeTeacherPanel||'feedback')===id?html:''}</section>`; }
teacherWorkspaceHtml = function(session, student, rec) { const mode = normalizeFeedbackMode(rec.feedbackMode || state.activeFeedbackMode); const feedbackBlocks = `<div class="feedback-toolbar"><div class="mode-switch">${FEEDBACK_MODES.map(item=>`<button data-mode="${item.key}" class="${mode===item.key?'active':''}">${item.label}${rec.feedbackDrafts?.[item.key]?'<em>已有草稿</em>':''}</button>`).join('')}</div><div class="feedback-actions"><button data-action="generate-feedback">重新生成</button><button data-action="copy-feedback">一键复制</button></div></div><label class="switch"><input id="include-homework" type="checkbox" ${rec.includeHomework?'checked':''}> 加入作业情况</label><textarea id="feedback-draft" rows="${(FEEDBACK_MODES.find(m=>m.key===mode)||FEEDBACK_MODES[1]).rows}">${escapeHtml(rec.feedbackDrafts?.[mode] || '')}</textarea><p id="copy-tip" class="copy-tip"></p>`; return `<section class="card session-head"><div><h2>教师工作台</h2><p>${escapeHtml(session.sessionName)} · ${escapeHtml(student.name)}</p></div><button data-action="go-classroom">返回课堂登记</button></section>${workspacePanel('feedback','简洁版 / 自然详细版 / 专业版家长反馈',feedbackBlocks)}${workspacePanel('homework','作业管理',homeworkHtml(session))}${workspacePanel('review','课后复盘',reviewHtml(session))}${workspacePanel('equipment','实验器材',reviewHtml(session))}${workspacePanel('handout','讲义问题',`<div class="review">${renderTagChecks('handoutIssues', HANDOUT_ISSUES, session.postClassReview?.handoutIssues||[])}<input id="handoutLinks" value="${escapeHtml(session.postClassReview?.handoutLinks||'')}" placeholder="关联讲次/模块/题号"></div>`)}${workspacePanel('todo','下节待办',`<div class="review">${renderTagChecks('nextLessonTasks', NEXT_TASKS, session.postClassReview?.nextLessonTasks||[])}<textarea id="teacherReflection" placeholder="教师自由复盘">${escapeHtml(session.postClassReview?.teacherReflection||'')}</textarea></div>`)}${workspacePanel('history','同讲义历史提醒',historicalTipsHtml(session) || '<p class="empty-summary">暂无同讲义历史提醒。</p>')}`; };
const simplifiedSessionHtml = sessionHtml;
sessionHtml = function(session, student, rec) { return state.page === 'teacher' ? teacherWorkspaceHtml(session, student, rec) : classroomHtml(session, student, rec); };

render = function() { state = normalizeState(state); const { grade, klass } = current(); const student = activeStudent(); const session = currentSession(); const rec = session && student ? ensureStudentRecord(session, student.id) : null; const prog = session && rec ? studentSessionProgress(session, rec) : {done:0,total:0}; document.querySelector('#app').innerHTML = `<div class="sticky-summary"><strong>${escapeHtml(klass?.name || '未选班级')}</strong><span>${escapeHtml(student?.name || '未选学生')}</span><span>${escapeHtml(session?.sessionName || (state.prepDraft?.sessionName || '未开始课堂'))}</span><b>${prog.done}/${prog.total}</b></div><header class="hero"><div><p class="eyebrow">本地原型 · 自动保存 · 数据安全备份</p><h1>${state.page==='home'?'课堂首页':state.page==='prep'?'课堂准备':state.page==='teacher'?'教师工作台':'课堂登记'}</h1></div></header>${toastHtml()}${state.page==='home'?homeHtml(grade,klass,student,session):state.page==='prep'?prepHtml():session && student && rec ? sessionHtml(session,student,rec) : '<section class="card empty">请先在首页选择班级和学生，并从准备页开始课堂。</section>'}${navHtml()}`; bindEvents(); bindReorganizedEvents(); bindSimplifiedEvents(); if (state.toast) { const old = state.toast; state.toast = ''; persist(); setTimeout(()=>{ if (state.toast === old) state.toast=''; }, 10); } };

const simplifiedHandleAction = handleAction;
handleAction = function(action) {
  if (action === 'new-session') { state.prepDraft = makePrepDraft('new'); state.page='prep'; state.activePrepTab='lesson'; persist(); render(); return; }
  if (action === 'continue-last') { state.prepDraft = makePrepDraft('continue'); state.page='prep'; state.activePrepTab='continue'; persist(); render(); return; }
  if (action === 'confirm-start-session') { const draft = state.prepDraft; if (!draft || !draft.selectedQuestions?.length) { alert('请先选择至少一道题目，再开始课堂。'); return; } if (!createAutoBackup('确认开始新课堂')) return; const s = createSession({ classId: state.classId, sessionDate: todayISO(), sessionName: draft.sessionName || '新课堂', plannedContent: draft.plannedContent || '', lessonObjectives: draft.lessonObjectives || '', selectedQuestions: draft.selectedQuestions }); state.sessionId=s.sessionId; state.page='classroom'; state.currentView='classroom'; state.prepDraft=null; state.expandedModuleIds=[]; refreshHistoricalTeachingTips(s); persist(); render(); return; }
  if (action === 'add-questions') { const s=currentSession(); state.prepDraft = makePrepDraft('append'); if (s) { state.prepDraft.sessionName = s.sessionName; state.prepDraft.plannedContent = s.plannedContent || ''; state.prepDraft.lessonObjectives = s.lessonObjectives || ''; } state.page='prep'; state.activePrepTab='lesson'; showToast('请选择要追加的题目，确认开始后会创建一节新的课堂，历史记录不会被修改。'); return; }
  if (action === 'add-homework') { const session=currentSession(); if (!session) return; const hw={homeworkItemId:uid('hw'), title:document.querySelector('#hw-title')?.value||'自定义作业', sourceType:document.querySelector('#hw-type')?.value || '自定义作业', lessonId:'', moduleId:'', questionIds:[], description:'', dueDate:document.querySelector('#hw-due')?.value||'', required:true, estimatedMinutes:'', createdFromSessionId:session.sessionId, createdAt:new Date().toISOString(), classification: classificationSnapshot(activeClass()||{})}; session.homeworkAssigned.push(hw); persist(); render(); return; }
  if (action === 'add-custom-selected') { const items = [...document.querySelectorAll('[data-custom-question]:checked')].map(input => { const [lid,mid,idx] = input.dataset.customQuestion.split(':'); const lesson=appData.lessons.find(l=>l.id===lid); const mod=lesson?.modules?.find(m=>m.id===mid); return lesson && mod ? sessionItemFromQuestion(lesson, mod, mod.questions[Number(idx)], Number(idx), '自定义选题') : null; }).filter(Boolean); addDraftItems(items); persist(); render(); return; }
  if (action === 'add-continue-selected') { const cont=continueCandidates(state.classId); const picked=new Set([...document.querySelectorAll('[data-continue-question]:checked')].map(x=>x.dataset.continueQuestion)); addDraftItems(cont.items.filter(x=>picked.has(x.item.sessionItemId)).map(x=>continuationClone(x.item, cont.last.sessionId))); state.prepDraft.sourceSessionId=cont.last?.sessionId || ''; persist(); render(); return; }
  if (action === 'go-teacher') { state.page='teacher'; state.currentView='teacher'; state.activeTeacherPanel ||= 'feedback'; persist(); render(); return; }
  if (action === 'go-classroom') { state.page='classroom'; state.currentView='classroom'; persist(); render(); return; }
  return simplifiedHandleAction(action);
};
function bindSimplifiedEvents() {
  document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{state.page=b.dataset.page; state.currentView=state.page==='teacher'?'teacher':'classroom'; persist(); render();});
  document.querySelectorAll('[data-prep-tab]').forEach(b=>b.onclick=()=>{state.activePrepTab=b.dataset.prepTab; persist(); render();});
  document.querySelector('#prep-session-name')?.addEventListener('input', e=>{state.prepDraft ||= makePrepDraft(); state.prepDraft.sessionName=e.target.value; persist();});
  document.querySelector('#prep-plan')?.addEventListener('input', e=>{state.prepDraft ||= makePrepDraft(); state.prepDraft.plannedContent=e.target.value; persist();});
  document.querySelector('#prep-objectives')?.addEventListener('input', e=>{state.prepDraft ||= makePrepDraft(); state.prepDraft.lessonObjectives=e.target.value; persist();});
  document.querySelectorAll('[data-add-lesson]').forEach(b=>b.onclick=()=>{ const lesson=appData.lessons.find(l=>l.id===b.dataset.addLesson); if (lesson && confirmCrossRange([lesson])) { addDraftItems(lessonItems(lesson)); persist(); render(); }});
  document.querySelectorAll('[data-add-module]').forEach(b=>b.onclick=()=>{ const [lid,mid]=b.dataset.addModule.split(':'); const lesson=appData.lessons.find(l=>l.id===lid); const mod=lesson?.modules?.find(m=>m.id===mid); if (lesson && mod && confirmCrossRange([lesson])) { addDraftItems(moduleDraftItems(lesson, mod)); persist(); render(); }});
  document.querySelectorAll('[data-remove-draft]').forEach(b=>b.onclick=()=>{ draftItems().splice(Number(b.dataset.removeDraft),1); renumberDraft(); showToast('已删除所选内容。'); });
  document.querySelectorAll('[data-move-draft]').forEach(b=>b.onclick=()=>{ const [raw,deltaRaw]=b.dataset.moveDraft.split(':'); const i=Number(raw), j=i+Number(deltaRaw); const items=draftItems(); if (j>=0 && j<items.length) { [items[i],items[j]]=[items[j],items[i]]; renumberDraft(); showToast('已调整题目顺序。'); } });
}
render();
